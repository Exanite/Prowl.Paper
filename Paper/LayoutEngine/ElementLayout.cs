...
namespace Prowl.PaperUI.LayoutEngine
{
    public static class ElementLayout
    {
        ...
        internal static UISize Layout(ElementHandle elementHandle, Paper gui, double pointUnitScale)
        {
            ...
            var size = DoLayout(elementHandle, LayoutType.Column, height, width, pointUnitScale);
        ...
        private static UISize DoLayout(ElementHandle elementHandle, LayoutType parentLayoutType, double parentMain, double parentCross, double pointUnitScale)
        {
            ref var element = ref elementHandle.Data;
            ...
            double minMain = main.IsStretch
                ? DEFAULT_MIN
                : GetMinMain(ref element, parentLayoutType).ToPx(parentMain, DEFAULT_MIN, pointUnitScale);

            double maxMain = main.IsStretch
                ? DEFAULT_MAX
                : GetMaxMain(ref element, parentLayoutType).ToPx(parentMain, DEFAULT_MAX, pointUnitScale);

            double minCross = cross.IsStretch
                ? DEFAULT_MIN
                : GetMinCross(ref element, parentLayoutType).ToPx(parentCross, DEFAULT_MIN, pointUnitScale);

            double maxCross = cross.IsStretch
                ? DEFAULT_MAX
                : GetMaxCross(ref element, parentLayoutType).ToPx(parentCross, DEFAULT_MAX, pointUnitScale);
            ...
            if (main.IsStretch)
                ...
                computedMain = main.ToPx(parentMain, 100f, pointUnitScale);
            // Auto stays at 0
            ...
            if(cross.IsStretch)
                ...
                computedCross = cross.ToPx(parentCross, 100f, pointUnitScale);
            // Auto stays at 0
            ...
            double borderMainBefore = borderMainBeforeUnit.ToPx(computedMain, DEFAULT_BORDER_WIDTH, pointUnitScale);
            var borderMainAfterUnit = GetBorderMainAfter(ref element, parentLayoutType);
            double borderMainAfter = borderMainAfterUnit.ToPx(computedMain, DEFAULT_BORDER_WIDTH, pointUnitScale);
            var borderCrossBeforeUnit = GetBorderCrossBefore(ref element, parentLayoutType);
            double borderCrossBefore = borderCrossBeforeUnit.ToPx(computedCross, DEFAULT_BORDER_WIDTH, pointUnitScale);
            var borderCrossAfterUnit = GetBorderCrossAfter(ref element, parentLayoutType);
            double borderCrossAfter = borderCrossAfterUnit.ToPx(computedCross, DEFAULT_BORDER_WIDTH, pointUnitScale);
            ...
            for (int i = 0; i < parentDirectedChildren.Count; i++)
            {
                ...
                if (childMainBefore.IsStretch)
                {
                    ...
                    mainAxis.Add(new StretchItem(
                        ...
                        childMinMainBefore.ToPx(actualParentMain, DEFAULT_MIN, pointUnitScale),
                        childMaxMainBefore.ToPx(actualParentMain, DEFAULT_MAX, pointUnitScale)
                    ));
                }
                ...
                if (childMain.IsStretch)
                {
                    ...
                    mainAxis.Add(new StretchItem(
                        ...
                        childMinMain.ToPx(actualParentMain, DEFAULT_MIN, pointUnitScale),
                        childMaxMain.ToPx(actualParentMain, DEFAULT_MAX, pointUnitScale)
                    ));
                }
                ...
                if (childMainAfter.IsStretch)
                {
                    ...
                    mainAxis.Add(new StretchItem(
                        ...
                        childMinMainAfter.ToPx(actualParentMain, DEFAULT_MIN, pointUnitScale),
                        childMaxMainAfter.ToPx(actualParentMain, DEFAULT_MAX, pointUnitScale)
                    ));
                }

                // Compute fixed-size child spaces
                double computedChildCrossBefore = childCrossBefore.ToPxClamped(
                    actualParentCross, 0f, childMinCrossBefore, childMaxCrossBefore, pointUnitScale);
                double computedChildCrossAfter = childCrossAfter.ToPxClamped(
                    actualParentCross, 0f, childMinCrossAfter, childMaxCrossAfter, pointUnitScale);
                double computedChildMainBefore = childMainBefore.ToPxClamped(
                    actualParentMain, 0f, childMinMainBefore, childMaxMainBefore, pointUnitScale);
                double computedChildMainAfter = childMainAfter.ToPxClamped(
                    actualParentMain, 0f, childMinMainAfter, childMaxMainAfter, pointUnitScale);

                double computedChildMain = 0f;
                double computedChildCross = childCross.ToPx(actualParentCross, 0f, pointUnitScale);
                ...
                {
                    var childSize = DoLayout(childHandle, layoutType, actualParentMain, actualParentCross, pointUnitScale);
            ...
            for (int i = 0; i < children.Count; i++)
            {
                ...
                if (childCrossBefore.IsStretch)
                {
                    double childMinCrossBefore = GetMinCrossBefore(ref child.Element.Data, layoutType)
                        .ToPx(actualParentCross, DEFAULT_MIN, pointUnitScale);
                    double childMaxCrossBefore = GetMaxCrossBefore(ref child.Element.Data, layoutType)
                        .ToPx(actualParentCross, DEFAULT_MAX, pointUnitScale);
                ...
                while (unfrozenMainCount > 0)
                {
                    ...
                    foreach (var item in mainAxis)
                    {
                        ...
                        if (item.ItemType == StretchItem.ItemTypes.Size)
                        {
                            ...
                            var childSize = DoLayout(child.Element, layoutType, actualMain, childCross, pointUnitScale);
                            child.Cross = childSize.Cross;
                            crossMax = Math.Max(crossMax, child.CrossBefore + child.Cross + child.CrossAfter);
            ...
            foreach (var childHandle in GetChildren(elementHandle))
            {
                ...
                double computedChildCrossBefore = childCrossBefore.ToPxClamped(
                    actualParentCross, 0f, GetMinCrossBefore(ref childHandle.Data, layoutType), GetMaxCrossBefore(ref childHandle.Data, layoutType), pointUnitScale);
                double computedChildCrossAfter = childCrossAfter.ToPxClamped(
                    actualParentCross, 0f, GetMinCrossAfter(ref childHandle.Data, layoutType), GetMaxCrossAfter(ref childHandle.Data, layoutType), pointUnitScale);
                double computedChildMainBefore = childMainBefore.ToPxClamped(
                    actualParentMain, 0f, GetMinMainBefore(ref childHandle.Data, layoutType), GetMaxMainBefore(ref childHandle.Data, layoutType), pointUnitScale);
                double computedChildMainAfter = childMainAfter.ToPxClamped(
                    actualParentMain, 0f, GetMinMainAfter(ref childHandle.Data, layoutType), GetMaxMainAfter(ref childHandle.Data, layoutType), pointUnitScale);
            ...
            {
                var child = children[i];
                ProcessChildCrossStretching(child, layoutType, actualParentCross, actualParentMain, pointUnitScale,
            ...
            {
                var child = children[i];
                ProcessChildMainStretching(child, layoutType, actualParentMain, actualParentCross, pointUnitScale,
            ...
            {
                ProcessChildCrossSpacing(child, layoutType, actualParentCross, pointUnitScale,
            ...
            int childIndex)
        {
            ...
            if (childCrossBefore.IsStretch)
            {
                double min = GetMinCrossBefore(ref child.Element.Data, layoutType).ToPx(parentCross, DEFAULT_MIN, pointUnitScale);
                double max = GetMaxCrossBefore(ref child.Element.Data, layoutType).ToPx(parentCross, DEFAULT_MAX, pointUnitScale);
            ...
            if (childCross.IsStretch)
            {
                double min = GetMinCross(ref child.Element.Data, layoutType).ToPx(parentCross, DEFAULT_MIN, pointUnitScale);
                double max = GetMaxCross(ref child.Element.Data, layoutType).ToPx(parentCross, DEFAULT_MAX, pointUnitScale);
            ...
            if (childCrossAfter.IsStretch)
            {
                double min = GetMinCrossAfter(ref child.Element.Data, layoutType).ToPx(parentCross, DEFAULT_MIN, pointUnitScale);
                double max = GetMaxCrossAfter(ref child.Element.Data, layoutType).ToPx(parentCross, DEFAULT_MAX, pointUnitScale);
            ...
            while (unfrozenCrossCount > 0)
            {
                ...
                foreach (var item in crossAxis)
                {
                    ...
                    if (item.ItemType == StretchItem.ItemTypes.Size && !GetMain(ref child.Element.Data, layoutType).IsStretch)
                    {
                        var childSize = DoLayout(child.Element, layoutType, parentMain, actualCross, pointUnitScale);
            ...
            int childIndex)
        {
            ...
            if (childMainBefore.IsStretch)
            {
                double min = GetMinMainBefore(ref child.Element.Data, layoutType).ToPx(parentMain, DEFAULT_MIN, pointUnitScale);
                double max = GetMaxMainBefore(ref child.Element.Data, layoutType).ToPx(parentMain, DEFAULT_MAX, pointUnitScale);
            ...
            if (childMain.IsStretch)
            {
                double min = GetMinMain(ref child.Element.Data, layoutType).ToPx(parentMain, DEFAULT_MIN, pointUnitScale);
                double max = GetMaxMain(ref child.Element.Data, layoutType).ToPx(parentMain, DEFAULT_MAX, pointUnitScale);
            ...
            if (childMainAfter.IsStretch)
            {
                double min = GetMinMainAfter(ref child.Element.Data, layoutType).ToPx(parentMain, DEFAULT_MIN, pointUnitScale);
                double max = GetMaxMainAfter(ref child.Element.Data, layoutType).ToPx(parentMain, DEFAULT_MAX, pointUnitScale);
        ...
        private static UISize DoLayout(ElementHandle elementHandle, LayoutType parentLayoutType, double parentMain, double parentCross, double pointUnitScale, double? fixedCross)
        {
            var size = DoLayout(elementHandle, parentLayoutType, parentMain, parentCross, pointUnitScale);
